...
export default function Search({userId, navigation}) {
  const [searchTerm, setSearchTerm] = useState('');
  const [location, setLocation] = useState(null);
  const [errorMsg, setErrorMsg] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  useEffect(() => {
    console.log('searchJS', userId, navigation.navigate)
    // console.log(options);
  }, [1])
  // useEffect(() => {
  //   (async () => {
  ...
  const handleSearch = () => {
    ...
    return fetch(`${URL}/${searchTerm}/portland`, {
      method: 'GET'
    })
      .then(response => response.json())
      // redirects to the Details component, passes the response as a prop so it is accessible in that component
      .then(response => navigation.push(`Details`, { search: response, name: `Details`, userId: userId }))
