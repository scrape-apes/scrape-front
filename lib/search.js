...
export default function Search({ navigation, location }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [errorMsg, setErrorMsg] = useState(null);
  const [isLoading, setIsLoading] = useState(false);

  // Move to utils file, import back into project
  // Known problem where request fails using iOS development, works using webpag
  const handleSearch = () => {
    setIsLoading(true)
    ...
    return fetch(`${URL}/${searchTerm}/${location[0].city}`, {
      ...
      .then(response => {
        return navigation.navigate(`Details`, { search: response, name: `Details` })
      })
